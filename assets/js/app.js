...
// 1. نظام التبديل بين الوضع المظلم والفاتح
        ...
        function initViewMap() {
            ...
            function safeLegendAppend(html) {
                const legend = document.getElementById('layoutLegendContent');
                if (!legend) return;
                const empty = legend.querySelector('.layout-legend-empty');
                if (empty) empty.remove();
                legend.insertAdjacentHTML('beforeend', html);
            }

            function rebuildLayoutLegend(items = []) {
                const legend = document.getElementById('layoutLegendContent');
                if (!legend) return;
                if (!items.length) {
                    legend.innerHTML = '<div class="layout-legend-empty text-xs text-gray-500 w-full col-span-2 text-center py-2">سيتم إضافة طبقات الخريطة هنا بمجرد اختيارها.</div>';
                    return;
                }
            ...
            function syncLayoutOverview() {
                if (!layoutMap || !layoutOverviewMap || !layoutOverviewRect) return;
                const bounds = layoutMap.getBounds();
                layoutOverviewRect.setBounds(bounds);
            }

            function fitLayoutOverviewToMain() {
                if (!layoutMap || !layoutOverviewMap) return;
                layoutOverviewMap.fitBounds(layoutMap.getBounds(), { padding: [12, 12] });
                syncLayoutOverview();
            }
            ...
            async function loadLayoutDefaultLayers() {
                ...
                try {
                    const [govRes, zoneRes] = await Promise.all([
                        fetch('gov.geojson'),
                        fetch('zone.geojson')
                    ]);
                    ...
                    L.geoJSON(govData, { style: govStyle }).addTo(layoutOverviewMap);
                    L.geoJSON(zoneData, { style: getZoneStyle }).addTo(layoutOverviewMap);
                    ...
                    rebuildLayoutLegend([
                        '<div class="flex items-center gap-2"><div class="w-4 h-4 rounded bg-blue-500 border border-white/50 shadow-sm"></div><span class="text-xs font-semibold text-gray-700">مناطق حصر جديدة</span></div>',
                        '<div class="flex items-center gap-2"><div class="w-4 h-4 rounded bg-yellow-500 border border-white/50 shadow-sm"></div><span class="text-xs font-semibold text-gray-700">قيد الحصر</span></div>',
                        '<div class="flex items-center gap-2"><div class="w-4 h-4 rounded bg-green-500 border border-white/50 shadow-sm"></div><span class="text-xs font-semibold text-gray-700">تم الانتهاء</span></div>',
                        '<div class="flex items-center gap-2"><div class="w-4 h-4 rounded bg-white border border-gray-900 shadow-sm"></div><span class="text-xs font-semibold text-gray-700">حدود المحافظات</span></div>',
                        '<div class="flex items-center gap-2 mt-1 col-span-2"><i class="fa-solid fa-satellite text-blue-500"></i><span class="text-xs font-semibold text-gray-700">صورة فضائية أساسية</span></div>'
                    ]);
            ...
            function initLayoutMap() {
                ...
                const layoutMapEl = document.getElementById('layoutMapInstance');
                const layoutOverviewEl = document.getElementById('layoutOverviewMap');
                if (!layoutMapEl || !layoutOverviewEl) return;
                
                layoutMap = L.map(layoutMapEl, {
                    zoomControl: false,
                    attributionControl: false
                }).setView([31.3547, 34.3088], 11);
            ...
            if(navBtnStyle) {
                navBtnStyle.addEventListener('click', () => {
                   initLayoutMap();
                });
            }

            const layoutMapOverlay = document.getElementById('layoutMapOverlay');
            const layoutMapInstanceDiv = document.getElementById('layoutMapInstance');
            const layoutMapActivateText = document.getElementById('layoutMapActivateText');
            ...
            function updateLayoutScaleBar() {
                ...
                const scaleTextValue = document.getElementById('scaleTextValue');
            ...
            const layoutGeoJsonInput = document.getElementById('layoutGeoJsonInput');
            if(layoutGeoJsonInput) {
                layoutGeoJsonInput.addEventListener('change', function(e) {
                    initLayoutMap();
                    ...
                    reader.onload = function(evt) {
                        try {
                            ...
                            safeLegendAppend(`
                                <div class="flex items-center gap-2"><div class="w-5 h-4 bg-red-500/20 border-2 border-red-500 shadow-sm"></div><span class="text-xs font-semibold text-gray-700">${file.name.substring(0, 20)}</span></div>
                            `);
                        } catch (err) {
            ...
            const layoutUrlBtn = document.getElementById('layoutUrlBtn');
            if(layoutUrlBtn) {
                layoutUrlBtn.addEventListener('click', () => {
                    initLayoutMap();
                    const url = prompt('أدخل رابط خادم الصور (مثال: https://mt1.google.com/vt/lyrs=s&x={x}&y={y}&z={z}):');
                    if(url && layoutMap) {
                        L.tileLayer(url, {maxZoom: 20}).addTo(layoutMap);
                        safeLegendAppend(`
                            <div class="flex items-center gap-2 mt-1 col-span-2"><i class="fa-solid fa-satellite text-blue-500"></i><span class="text-xs font-semibold text-gray-700">طبقة XYZ خارجية</span></div>
                        `);
                    }
                });
            }
            ...
            function setupDraggableLayoutElements() {
                ...
                let activeEl = null, startX, startY, initX, initY;

                draggables.forEach(el => {
                    el.addEventListener('mousedown', (e) => {
                        document.querySelectorAll('.layout-draggable').forEach(d => {
                            d.classList.remove('outline', 'outline-2', 'outline-blue-500');
                        });
                        ...
                        initX = parseInt(compStyle.left) || 0;
                        initY = parseInt(compStyle.top) || 0;
                        el.style.right = 'auto';
            ...
            const colorPicker = document.querySelector('#propNorthArrow input[type="color"]');
            if(colorPicker) {
                colorPicker.addEventListener('input', (e) => {
                    const svg = document.getElementById('northArrowSvg');
                    if(svg) {
                        svg.style.setProperty('--na-color', e.target.value);
                    }
                });
            }
            ...
            if(rotationInput) {
                rotationInput.addEventListener('input', (e) => {
                    const naBox = document.getElementById('layoutNorthArrow');
                    if(naBox) {
                        naBox.style.transform = `rotate(${e.target.value}deg)`;
                    }
                });
            }

            const layoutZoomInBtn = document.getElementById('layoutZoomInBtn');
            const layoutZoomOutBtn = document.getElementById('layoutZoomOutBtn');
            const layoutFullscreenBtn = document.getElementById('layoutFullscreenBtn');
            const layoutOverviewResetBtn = document.getElementById('layoutOverviewResetBtn');
            const btnAddLayoutNorthArrow = document.getElementById('btnAddLayoutNorthArrow');
            const btnAddLayoutScaleBar = document.getElementById('btnAddLayoutScaleBar');
            const btnAddLayoutLegend = document.getElementById('btnAddLayoutLegend');
            const layoutNorthArrow = document.getElementById('layoutNorthArrow');
            const layoutScaleBar = document.getElementById('layoutScaleBar');
            const layoutLegend = document.getElementById('layoutLegend');
            const layoutPaper = document.querySelector('#style [class*="group/canvas"]') || document.querySelector('#style .aspect-\\[1\\.414\\/1\\]');
            const layoutCanvasBox = document.querySelector('#style .glass-panel.rounded-3xl.p-4');

            if (layoutZoomInBtn) {
                layoutZoomInBtn.addEventListener('click', () => {
                    initLayoutMap();
                    if (layoutMap) layoutMap.zoomIn();
                });
            }
            if (layoutZoomOutBtn) {
                layoutZoomOutBtn.addEventListener('click', () => {
                    initLayoutMap();
                    if (layoutMap) layoutMap.zoomOut();
                });
            }
            if (layoutFullscreenBtn && layoutCanvasBox) {
                layoutFullscreenBtn.addEventListener('click', async () => {
                    try {
                        if (!document.fullscreenElement) {
                            await layoutCanvasBox.requestFullscreen();
                            layoutFullscreenBtn.innerHTML = '<i class="fa-solid fa-compress"></i>';
                        } else {
                            await document.exitFullscreen();
                            layoutFullscreenBtn.innerHTML = '<i class="fa-solid fa-expand"></i>';
                        }
                        setTimeout(() => {
            ...
            if (layoutOverviewResetBtn) {
                layoutOverviewResetBtn.addEventListener('click', (e) => {
                    e.preventDefault();
                    initLayoutMap();
                    fitLayoutOverviewToMain();
                });
            ...
            bindLayoutToggle(btnAddLayoutNorthArrow, layoutNorthArrow);
            bindLayoutToggle(btnAddLayoutScaleBar, layoutScaleBar);
            bindLayoutToggle(btnAddLayoutLegend, layoutLegend);

            if (document.getElementById('style')?.classList.contains('active')) {
                initLayoutMap();
            }
            // تصدير صورة من مساحة الإخراج الفني
            const btnExportPDF = document.getElementById('btnExportPDF');
            if(btnExportPDF) {
                btnExportPDF.addEventListener('click', async () => {
                   const originalText = btnExportPDF.innerHTML;
                   btnExportPDF.innerHTML = '<i class="fa-solid fa-spinner fa-spin ml-2"></i> جاري تجهيز الملف...';
                   try {
                       const paperElement = document.querySelector('#style [class*="group/canvas"]') || document.querySelector('#style .aspect-\\[1\\.414\\/1\\]');
                       if (!paperElement || typeof html2canvas === 'undefined') throw new Error('html2canvas not available');
                       ...
                       link.click();
                       btnExportPDF.innerHTML = '<i class="fa-solid fa-check ml-2"></i> تم التصدير';
                   } catch (err) {
                       console.error('Export error:', err);
                       btnExportPDF.innerHTML = '<i class="fa-solid fa-triangle-exclamation ml-2"></i> تعذر التصدير';
                   }
                   setTimeout(() => { btnExportPDF.innerHTML = originalText; }, 2500);
                });
            }
